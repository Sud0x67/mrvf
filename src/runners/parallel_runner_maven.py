...
class ParallelRunnerMaven:
    ...
    def __init__(self, args, logger):
        ...
        for worker_conn in self.worker_conns:
            ...
            # env_fn 产生env环境， CloudpickleWrapper 托管了环境，cloudxx.x 即env
            self.ps.append(ps)
        for p in self.ps:
            p.daemon = True
            p.start()
        ...
        while True:
            ...
            for idx, parent_conn in enumerate(self.parent_conns):
                if not terminated[idx]:
                    ...
                    if data["terminated"] and not data["info"].get("episode_limit", False):  # 此处and后面的判断没什么用
                        env_terminated = True
